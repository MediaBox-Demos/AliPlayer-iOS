//
//  AUIShortDramaListViewController.m
//  AUIPlayer
//
//  Created by keria on 2024/11/6.
//

#import "AUIShortDramaListViewController.h"
...
// 短剧剧集信息列表示例URL常量
static NSString * kDramaInfoListSampleUrl = @"https://alivc-demo-cms.alicdn.com/versionProduct/resources/shortdrama/drama-info-list-default.json";
...
@interface AUIShortDramaListViewController () <UIScrollViewDelegate, UIGestureRecognizerDelegate, AUITransparentTabBarViewDelegate, AUIShortVideoDataProviderDelegate, UINavigationControllerDelegate>
...
// 自定义过渡动画
@property (nonatomic, strong) AUICustomTransitionAnimator *anim;

@end

@implementation AUIShortDramaListViewController

#pragma mark - Class Methods

// 设置剧集信息的URL
+ (void)setDramaInfoURL:(NSString *)dramaInfoURL {
    kDramaInfoListSampleUrl = dramaInfoURL;
}
...
// 类的首次加载
+ (void)initialize {
    if (self == [AUIShortDramaListViewController class]) {
        // 在类第一次加载时初始化静态数组
        kSegmentedControlItems = @[AVGetString(@"Detail", @"AUIShortDramaList"), AVGetString(@"Recommend", @"AUIShortDramaList")];
    }
}
...
- (void)viewDidLoad {
    ...
    self.anim = [[AUICustomTransitionAnimator alloc] init];
...
- (UIViewController *)viewControllerForItem:(NSString *)item {
    if ([AVGetString(@"Detail", @"AUIShortDramaList") isEqualToString:item]) {
        return [[AUIShortDramaDetailViewController alloc] initWithDataProvider:self];
    } else if ([AVGetString(@"Recommend", @"AUIShortDramaList") isEqualToString:item]) {
        return [[AUIShortDramaRecommendViewController alloc] initWithDataProvider:self];
    }
...
- (void)loadData:(id)controller {
    ...
    loading.labelText = AVGetString(@"Loading...", @"AUIShortDramaList");
    
    // 创建弱引用，以避免循环引用
    __weak typeof(self) weakSelf = self;
    [AUIShortDramaListDataManager requestDramaInfoList:kDramaInfoListSampleUrl completed:^(NSArray<AUIShortDramaInfo *> * _Nullable data, NSError * _Nullable error) {
        // 获取强引用以避免过早释放 self
        ...
        [strongSelf.dramaInfoList addObjectsFromArray:data];
        
        // 调用相应子视图控制器的 appendDramaInfoList: 方法
        if (controller && [controller respondsToSelector:@selector(appendDramaInfoList:)]) {
            [controller appendDramaInfoList:[data copy]];
        }
    }];
}
